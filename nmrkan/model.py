...
class DenseKanLayer(torch.nn.Module):
    def __init__(self, N_input:int, 
        ...
        self.regression_layer=torch.nn.Conv1d(in_channels=N_output, out_channels=N_output, kernel_size=len(activations),groups=N_output,bias=False)
    def forward(self, x: torch.Tensor):
        ...
        if self.normalize:
            with torch.no_grad():
                normmap=(torch.sum(self.regression_layer.weight.data,dim=2,keepdim=True)>-1.0).float()
                self.regression_layer.weight.data=self.regression_layer.weight.data*(1-normmap) + self.regression_layer.weight.data*(normmap)/(torch.sum(self.regression_layer.weight.data,dim=2,keepdim=True))
    ...
    def simplify(self,threshold=1e-2) ->None:
        ...
        """
        self.regression_layer.weight.data[torch.abs(self.regression_layer.weight.data)<threshold]=0
        with torch.no_grad():
            self.linear.weight.data=(self.regression_layer.weight.data.reshape(1,-1).abs()>threshold).T*self.linear.weight.data
        return None
    ...
    def anneal_linear(self,anneal_rate=1e-1):
        ...
        with torch.no_grad():
            ...
            self.linear.weight.data=self.linear.weight.data+torch.randn_like(self.linear.weight.data)*anneal_rate*norm
        return None
    def anneal_regression(self,anneal_rate=1e-1):
        ...
        """
        with torch.no_grad():
            self.regression_layer.weight.data=self.regression_layer.weight.data+torch.randn_like(self.regression_layer.weight.data)*anneal_rate
            self.regression_layer.weight.data=self.regression_layer.weight.data.abs()/torch.sum(self.regression_layer.weight.data.abs(),dim=2,keepdim=True)
        return None
...
class KharKAN(torch.nn.Module):
    def __init__(self, layers:Tuple[int]=None,
        ...
        self.layers=torch.nn.ModuleList([DenseKanLayer(layers[i],layers[i+1],activations,activations_reprs,normalize) for i in range(len(layers)-1)])
    def forward(self, x):
    ...
    def symbolic_formula(self):
        ...
        max_length=max(weight.shape[0] for layer in self.layers for weight in layer.linear.weight)
        self.x_to_y_dicts={f"x_{i}":f"y_{i}" for i in range(max_length)}
        self.y_to_x_dicts={f"y_{i}":f"x_{i}" for i in range(max_length)}
        for i in range(1,len(self.layers)):
            self.subs_dict[i]={f"x_{j}":formula[f"layer_{i-1}"][f"z_{j}"] for j in range(len(formula[f"layer_{i-1}"]))}
            self.subs_dict[i]={k:v.subs(self.x_to_y_dicts) for k,v in self.subs_dict[i].items()}
        finalformula=formula[f"layer_{len(self.layers)-1}"]
        for i in range(len(self.layers)-1,0,-1):
            for j in range(len(finalformula)):
                finalformula[f"z_{j}"]=finalformula[f"z_{j}"].subs(self.subs_dict[i]).subs(self.y_to_x_dicts)
        return finalformula
    def anneal(self,anneal_rate=1e-1):
        for layer in self.layers:
