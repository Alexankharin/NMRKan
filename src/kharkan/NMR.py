...
from src.datagen.data_generator import (
...
def get_frequences(Jintra: np.ndarray, deltaJ: np.ndarray = None) -> list:
    """
...
def get_frequences_ordered(Jintra: np.ndarray, deltaJ: np.ndarray = None) -> list:
    """
    Computes the transition frequencies for a batch of Hamiltonian matrices,
    ensuring the results are sorted.
    ...
    Returns:
    list: List of sorted transition frequencies for each input pair.
    """
    # Handle input: allow (B,2) or two arrays of shape (B,)
    if deltaJ is None:
        Jintra = np.asarray(Jintra)
